  ...
  useEffect(() => {
    try {
      const fetchProjects = async () => {
        const data = await fetchServerData("/api/archive");
        ...
        <div className="content">
          ...
          <div className="grid">
            ...
            <div className="panel">
              ...
              <p className="p-desc">
                5-part welcome flow for a fintech startup.
              </p>
              <div className="tag-row">
      ...
      >
        ...
        <div {...aosFadeUp}>
          <p className="flex items-center gap-2 text-purple-200">
            <GoDotFill />
            <span className="uppercase font-extralight text-xs tracking-widest">
              Selected work
            </span>
          </p>
          <h2 className="text-left text-2xl md:text-3xl font-bold leading-8 text-gray-200 flex items-center justify-start mt-3">
            <p {...aosFadeUp}>
              Frontend, backend & <br />{" "}
              <span className="text-cyan-400">email</span> — crafted with care
            </p>
          </h2>
        ...
        {/* Archive cards */}
        {Array.isArray(archives) && archives.length ? (
          <div className="grid auto-rows-fr gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {archives.slice(0, showMore).map(archive => (
              <Card
                key={uuidv4()}
                title={archive.title}
          ...
          // Show pulse before archive load
        ...
        {/* View all proejcts button */}
        <div className="flex justify-center">
          <Link
            {...aosZoomIn}
            href="https://taqwaitagency.com"
            className="group relative btn mr-2 mt-8 transition-all duration-500 md:mt-16 block tracking-wide hover:text-purple-500 px-3 py-2 text-xs text-gray-200 font-semibold"
            aria-label="Button for show more"
          >
            <span className="absolute left-0 bottom-0 w-6 h-5 border-l-2 border-b-2 border-gray-400 border-opacity-40 transition-all duration-1000 group-hover:border-opacity-100 group-hover:border-purple-500 font-semibold" />
            View all projects
            <span className="absolute right-0 top-0 w-6 h-5 border-r-2 border-t-2 border-gray-400 border-opacity-40 transition-all duration-1000 group-hover:border-opacity-100 group-hover:border-purple-500" />
          </Link>
